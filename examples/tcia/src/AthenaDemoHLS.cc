...
std::string AthenaDemoHLS::runQuery1(std::string treatment)
{
...
std::string AthenaDemoHLS::runQuery2(int age_low, int age_high)
{
    AthenaDemoHLS::Histogram histo;
    std::stringstream res;
    ...
    for (auto& treat:histo) {
        res << treat.first << ": " << treat.second.first << std::endl;
    }

    return res.str();
}

std::string AthenaDemoHLS::runQuery3(std::string name)
{
    AthenaDemoHLS::Histogram histo;
    std::stringstream res;
    ...
    if (!flag)
        res << "Drug Not found" << std::endl;

    return res.str();
}

std::string AthenaDemoHLS::runQuery4(std::string patient)
{
    ...
    for (int i = 0; i < res.size(); ++i) {
        try{
            res.at(i).store("results/crop_" + patient + std::to_string(i),
                            VCL::PNG);
        }
        catch (VCL::Exception e) {
            printf("[Exception] %s at %s:%d\n", e.name, e.file, e.line);
...
std::string AthenaDemoHLS::runQuery5(std::string patient)
{
    float threshold = 150;
    ...
    {
        try{
            res.at(i).store("results/thre_"+ patient + std::to_string(i),
                            VCL::PNG);
        }
        catch (VCL::Exception e) {
            printf("[Exception] %s at %s:%d\n", e.name, e.file, e.line);
...
std::string AthenaDemoHLS::runQuery6(std::string patient)
{
    float threshold = 150;
    ...
    {
        try{
            res.at(i).store("results/crop_thre_"+ patient + std::to_string(i),
                            VCL::PNG);
        }
        catch (VCL::Exception e) {
            printf("[Exception] %s at %s:%d\n", e.name, e.file, e.line);
